CDECK  ID>, SIGMCA.
       SUBROUTINE SIGMCA(X1,Y1,Z1,NETOT,NITOT,STAT,
     -      NHIST,IHIST,ITYPE,IENTRY,OPTION)
*-----------------------------------------------------------------------
*   SIGMCA - Subroutine that computes a drift line using a Monte-Carlo
*            technique to take account of diffusion and of avalanche
*            formation. Adds optionally the induced currents.
*   VARIABLES :
*   REFERENCE :
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      IVECU,IVECN,NEW,NMAX
       LOGICAL LELEPL,LIONPL,LTOWN,LATTA,STAT(4),CROSS,LESIG,LISIG,
     -      LPRINT
       CHARACTER*(*) OPTION
       EXTERNAL GASTWN,GASATT
*** Identify the routine if requested.
       IF(LIDENT)PRINT *,' /// ROUTINE SIGMCA ///'
*** Initial debugging output.
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ SIGMCA DEBUG   : MC drift'',
     -      '' from ('',E15.8,'','',E15.8,'','',E15.8,'')'')') X1,Y1,Z1
*** Make sure that electron drift velocities are available.
       IF(.NOT.GASOK(1))THEN
            PRINT *,' !!!!!! SIGMCA WARNING : Electron drift velocity'//
...
*** Obtain the matrix to store the avalanche development.
       ...
       IF(IFAIL.NE.0)THEN
            PRINT *,' !!!!!! SIGMCA WARNING : Unable to obtain'//
...
*** Default settings of parameters.
       PROBTH=0.01
*** Decode the options.
       IF(INDEX(OPTION,'NOION-TAIL').NE.0)THEN
            LISIG=.FALSE.
       ELSEIF(INDEX(OPTION,'ION-TAIL').NE.0)THEN
            LISIG=.TRUE.
       ENDIF
       IF(INDEX(OPTION,'NOELECTRON-PULSE').NE.0)THEN
            LESIG=.FALSE.
       ELSEIF(INDEX(OPTION,'ELECTRON-PULSE').NE.0)THEN
            LESIG=.TRUE.
       ENDIF
       IF(INDEX(OPTION,'NOCROSS')+INDEX(OPTION,'DIRECT').NE.0)THEN
            CROSS=.FALSE.
       ELSEIF(INDEX(OPTION,'CROSS').NE.0)THEN
            CROSS=.TRUE.
       ENDIF
       ...
       ELSEIF(INDEX(OPTION,'PLOT-ION').NE.0)THEN
            IF(.NOT.GASOK(2))THEN
                 PRINT *,' !!!!!! SIGMCA WARNING : Ion mobilities are'//
     -                ' absent; can not compute ion drift lines.'
       ...
       ELSEIF(INDEX(OPTION,'TOWNSEND').NE.0.AND..NOT.GASOK(4))THEN
            PRINT *,' !!!!!! SIGMCA WARNING : Townsend data is not'//
     -           ' present; TOWNSEND option not valid.'
       ...
       ELSEIF(INDEX(OPTION,'ATTACHMENT').NE.0.AND..NOT.GASOK(6))THEN
            PRINT *,' !!!!!! SIGMCA WARNING : Attachment data is not'//
...
*** Make sure that some kind of output has been requested.
       IF(.NOT.(LATTA.OR.LTOWN))THEN
            PRINT *,' !!!!!! SIGMCA WARNING : Neither attachment not'//
...
*   Verify that there still is an electron.
     ...
     -           ITYPE,IENTRY,2)
            IF(LPRINT)WRITE(LUNOUT,'(''  Electron '',I5,''/'',I5,
     -           '' attached at t='',E15.8)')
     -           J,IMCA,REAL(TU(I)+TU(I+1))/2
            GOTO 60
       ENDIF
*   Next subdivision.
50     CONTINUE
...
*   And enter the newly created electrons in the histograms.
     ...
     -           ITYPE,IENTRY,1)
            IF(LPRINT)WRITE(LUNOUT,'(''  Electron '',I5,''/'',I5,
     -           '' creates '',I5,'' pairs called '',I5,
     -           '' at t='',E15.8)')
     -           J,IMCA,NLIST(NMCA),NMCA,TLIST(NMCA)
       ENDIF
*** Also compute the newly produced ions if requested.
       IF(NION.GE.1.AND.(LIONPL.OR.STAT(4).OR.LISIG))THEN
*   Store offset time.
            TOFF=TU(I+1)
...
90          CONTINUE
*   Add the signals.
            IF(LISIG)CALL SIGADS(0.0,CROSS,1.0,IFAIL)
*   Enter the ion end point in the histograms if requested.
     ...
     -           NHIST,IHIST,ITYPE,IENTRY,4)
            IF(LPRINT)WRITE(LUNOUT,'(''  Ion      '',I5,''/'',I5,
     -           '' stops at t='',E15.8)') J,IMCA,TU(NU)
...
70     CONTINUE
*   Add the signals.
       IF(LESIG)CALL SIGADS(0.0,CROSS,1.0,IFAIL)
