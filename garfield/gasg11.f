CDECK  ID>, GASG11.
       SUBROUTINE GASG11(IFAIL)
*-----------------------------------------------------------------------
*   GASG11 - This routine retrieves the gas data written to an external
...
*   (Last changed on  2/ 8/10. Renamed on 18/ 9/11.)
*-----------------------------------------------------------------------
       implicit none
...
*** Identify the routine.
       IF(LIDENT)PRINT *,' /// ROUTINE GASG11 ///'
...
*** Open a dataset and inform DSNLOG.
       ...
       IF(IFAIL1.NE.0)THEN
            PRINT *,' !!!!!! GASG11 WARNING : Opening ',FILE(1:NCFILE),
     -           ' failed ; gas data are not read.'
            RETURN
       ENDIF
       CALL DSNLOG(FILE,'Gas data  ','Sequential','Read only ')
       IF(LDEBUG)PRINT *,' ++++++ GASG11 DEBUG   : Dataset ',
...
*   Locate the pointer on the header of the requested member.
       ...
       IF(.NOT.EXIS)THEN
            ...
            IF(EXIS)THEN
                 PRINT *,' ###### GASG11 ERROR   : Gas description '//
...
*   Check that this member contains indeed gas data.
       ...
       IF(LDEBUG)THEN
            PRINT *,' ++++++ GASG11 DEBUG   : Dataset header'//
     -           ' record follows:'
            PRINT *,STRING
       ENDIF
...
*   Check the version.
       ...
       IF(STRING(1:14).EQ.' Version   : 8')THEN
            PRINT *,' ------ GASG11 MESSAGE : This member is'//
     ...
     -           ' written in earlier format 9.'
            ...
            CALL GASG09(IFAIL)
            RETURN
       ELSEIF(STRING(1:14).EQ.' Version   : 10')THEN
            PRINT *,' ------ GASG11 MESSAGE : This member is'//
     -           ' written in earlier format 10.'
            CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
            CALL GASG09(IFAIL)
            RETURN
       ELSEIF(STRING(1:15).NE.' Version   : 11')THEN
            PRINT *,' !!!!!! GASG11 WARNING : This member'//
...
*   Excitation and ionisation identification strings.
       DO 10 I=1,NEXGAS
       READ(12,'(19X,A45,2X,2E15.8)',IOSTAT=IOS,ERR=2010)
     -      DSCEXG(I),ENEXG(I),PENPRB(I)
       PENRMS(I)=0
       PENDT(I)=0
10     CONTINUE
       DO 20 I=1,NIOGAS
...
*   Frequency scaling
       IF(TAB2D)THEN
            ...
            DO 330 K=1,NBTAB
            DO 340 L=1,NEXGAS
            EXGAS2(I,J,K,L)=EXGAS2(I,J,K,L)/PGAS
340         CONTINUE
            DO 350 L=1,NIOGAS
            IOGAS2(I,J,K,L)=IOGAS2(I,J,K,L)/PGAS
350         CONTINUE
330         CONTINUE
320         CONTINUE
310         CONTINUE
       ELSE
            DO 360 I=1,NGAS
            DO 370 L=1,NEXGAS
            EXGAS(I,L)=EXGAS(I,L)/PGAS
370         CONTINUE
            DO 380 L=1,NIOGAS
            IOGAS(I,L)=IOGAS(I,L)/PGAS
380         CONTINUE
360         CONTINUE
       ENDIF
*   Heed initialisation data.
       CALL GASHGT(IFAIL1)
       IF(IFAIL1.NE.0)THEN
            PRINT *,' !!!!!! GASG11 WARNING : Reading Heed data'//
...
*   SRIM initialisation data.
       CALL GASSG8(IFAIL1)
       PRINT *,' !!!!!! GASG08 WARNING : SRIM data from version 11'//
...
2000   CONTINUE
       PRINT *,' ###### GASG11 ERROR   : EOF encountered while reading',
     -      ' '//FILE(1:NCFILE)//' from unit 12 ; no gas data read.'
       CALL INPIOS(IOS)
       CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
       RETURN
2010   CONTINUE
       PRINT *,' ###### GASG11 ERROR   : Error while reading'//
     -      ' '//FILE(1:NCFILE)//' from unit 12 ; no gas data read.'
       CALL INPIOS(IOS)
       CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
       RETURN
2030   CONTINUE
       PRINT *,' ###### GASG11 ERROR   : Dataset '//FILE(1:NCFILE)//
