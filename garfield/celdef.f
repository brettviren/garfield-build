CDECK  ID>, CELDEF.
       SUBROUTINE CELDEF(IFAIL)
*-----------------------------------------------------------------------
*   CELDEF - Routine controling the flow of the cell-definition routines
*            it calls CELINP ,CELPRT, CELPLT, CELCHK, SETUP and CELTYP.
*   VARIABLES : IGET      : 1 if cell was read from dataset, 0 else.
...
*** Write the header of the cell section.
       WRITE(*,'(''1'')')
       PRINT *,' ================================================'
       PRINT *,' ==========  Start of cell definition  =========='
       PRINT *,' ================================================'
       PRINT *,' '
*** Read the cell data, IGET tells the origin of the data.
       IGET=0
       CALL CELINP(IGET,IFAIL)
       IF(IFAIL.NE.0)THEN
            PRINT *,' ###### CELDEF ERROR   : Your cell description'//
     -           ' can not be processed ; no cell data.'
            RETURN
       ENDIF
       IF(IGET.EQ.1)GOTO 10
*** Check that the cell makes sense.
       CALL CELCHK(IFAIL)
       IF(IFAIL.EQ.1)THEN
            PRINT *,' ###### CELDEF ERROR   : Your cell does not'//
...
*** Determine the cell type.
       CALL CELTYP(IFAIL)
       IF(IFAIL.NE.0)THEN
            PRINT *,' ###### CELDEF ERROR   : Type identification'//
...
*** Calculate the charges.
       CALL SETUP(IFAIL)
       IF(IFAIL.NE.0)THEN
            PRINT *,' ###### CELDEF ERROR   : Cell preparation is'//
...
*** Assign default strip widths.
       CALL CELSTR(IFAIL)
       IF(IFAIL.EQ.1)THEN
            PRINT *,' ###### CELDEF ERROR   : Strip preparation'//
     -           ' failed; end of this cell section.'
            RETURN
       ENDIF
10     CONTINUE
*** Output the cell data to a printer or a plotter, if requested.
       IF(LCELPR)CALL CELPRT
       IF(LCELPL)CALL CELPLT
*** Write the complete cell data in compact format to a dataset.
       CALL CELWRT(2)
*** Preselect read-out electrodes, see whether there are S electrodes.
       EXIST=.FALSE.
*   Consider wires.
       DO 20 I=1,NWIRE
...
*   Selection proper.
       IF(EXIST)THEN
            PRINT *,' ------ CELDEF MESSAGE : Pre-selecting the'//
     -           ' S electrodes.'
            CALL CELSEL('S')
       ELSE
            PRINT *,' ------ CELDEF MESSAGE : No S electrodes in the'//
...
*** Define the default drift area.
       ...
       IF(POLAR.AND.DYMAX-DYMIN.GE.2.0*PI)THEN
            ...
            PYMAX=+PI
       ENDIF
*** Set the default projection method.
       CALL PLAINT
       IF(POLAR)THEN
            PRVIEW='R-PHI'
       ELSE
