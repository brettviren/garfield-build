CDECK  ID>, FOPTE1.
       SUBROUTINE FOPTE1(M,U1,F1,XAUX)
*-----------------------------------------------------------------------
*   FOPTE1 - Integrates the energy over the deflection path.
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      LFGRAV,LFELEC,LFEXTR,LFWARN,LZROPR,LFITER
       DOUBLE PRECISION U1(*),F1(*),XAUX(*),COOR,BEND(2),DBEND(2),F0(2),
     -      DDX,DDY
       INTEGER L,M
       COMMON /FEODAT/ DDX,DDY
*** Loop over the wire delections.
       DO 10 L=1,M
*   Obtain the deflection.
       XAUX(1)=U1(L)
*   Set the sag parameters.
       COOR=XAUX(2)
       BEND(1)=DDX*XAUX(1)/SQRT(DDX**2+DDY**2)
       BEND(2)=DDY*XAUX(1)/SQRT(DDX**2+DDY**2)
       DBEND(1)=0
       DBEND(2)=0
*   Compute the force.
       CALL OPTSTP(COOR,BEND,DBEND,F0)
*   Take the component in the direction of the bend.
       F1(L)=(F0(1)*DDX+F0(2)*DDY)/SQRT(DDX**2+DDY**2)
10     CONTINUE
       END
