CDECK  ID>, MAPFMA.
       SUBROUTINE MAPFMA(FMAP,NCMAP,IDATA,IWMAP,DELBKG,UNITD,IFAIL)
*-----------------------------------------------------------------------
*   MAPFMA - Reads Ansys 2nd order tetrahedrons (which do not need to
*            have straight sides). Should work with Version 11.x and
*            Version 12.1
*   (Last changed on 13/ 1/12.)
*-----------------------------------------------------------------------
       implicit none
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      IDATA,NCAUX,IEND,IP,NTETRA,NPOINT,
     -      LOOKUP(10),NDELET,IMAT,NMAT,NTEMP
C       INTEGER IDUM
...
*   Open the materials file.
       IF(EXIST)THEN
            ...
            IF(IFAIL1.NE.0)THEN
                 PRINT *,' !!!!!! MAPFMA WARNING : Unable to open the'//
...
*   Read the number of materials.
            ...
            IF(NMAT.GT.MXEPS)THEN
                 PRINT *,' !!!!!! MAPFMA WARNING : Material file '//
...
*   Debugging
            IF(LDEBUG)THEN
                 WRITE(LUNOUT,'(''  ++++++ MAPFMA DEBUG   : Number'',
     -                '' of materials found: '',I10/26X,
     -                ''     i       Epsilon    Resistance'')') NEPS
                 DO 150 I=1,NEPS
                 WRITE(LUNOUT,'(27X,I5,2X,E12.5,2X,E12.5)')
     -                I,EPSMAT(I),OHMMAT(I)
150              CONTINUE
            ENDIF
       ENDIF
...
*   Increment element counter
       IF(NTETRA.GE.MXMAP)THEN
            PRINT *,' !!!!!! MAPFMA WARNING : Number of'//
...
C       READ(12,'(I8,5I4,1X,8I6/29X,2I6)',
C     -      END=2005,ERR=2010,IOSTAT=IOS)
C     -      ITETRA,IMAT,IDUM,IDUM,IDUM,IDUM,
C     -      IT1,IT2,IT3,IT4,IT5,IT6,IT7,IT8,IT9,IT10
C       READ(12,'(I8,5I4,1X,8I8/29X,2I8)',
C     -      END=2005,ERR=2010,IOSTAT=IOS)
C     -      ITETRA,IMAT,IDUM,IDUM,IDUM,IDUM,
C     -      IT1,IT2,IT3,IT4,IT5,IT6,IT7,IT8,IT9,IT10
C       READ(12,*,END=2005,ERR=2010,IOSTAT=IOS)
C     -      ITETRA,IMAT,IDUM,IDUM,IDUM,IDUM,
C     -      IT1,IT2,IT3,IT4,IT5,IT6,IT7,IT8,IT9,IT10
       READ(12,'(A)',END=2000,ERR=2010,IOSTAT=IOS) STRING
       STRING(13:28)=' '
       READ(STRING,*,END=2005,ERR=2010,IOSTAT=IOS)
     -      ITETRA,IMAT,IT1,IT2,IT3,IT4,IT5,IT6,IT7,IT8
       READ(12,*,END=2005,ERR=2010,IOSTAT=IOS) IT9,IT10
       IF(ITETRA.NE.NTETRA.AND..NOT.DELBKG)THEN
            PRINT *,' !!!!!! MAPFMA WARNING : Numbering mismatch'//
     -           ' at tetrahedron ',NTETRA,' in '//FAUX(1:NCAUX)//
     -           ' ; file not read.'
            CALL INPSWI('RESTORE')
            RETURN
       ENDIF
*   Store the material flag.
       IF(.NOT.LMPFIL)THEN
            IF(IMAT.LT.1.OR.IMAT.GT.MXEPS)THEN
                 PRINT *,' !!!!!! MAPFMA WARNING : Found out of range'//
     ...
     -                FAUX(1:NCAUX)//'; file not read.'
            ...
            ELSE
                 ...
                 IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ MAPFMA DEBUG   :'',
...
*   Store the reference pointers temporarily in Ex (point) and Ey (V).
       ...
       EYMAP(NTETRA,LOOKUP(9))=IT9
       EYMAP(NTETRA,LOOKUP(10))=IT10
...
2000   CONTINUE
       CLOSE(12,ERR=2030,IOSTAT=IOS)
       IF(LDEBUG)WRITE(LUNOUT,'(''  +++++++ MAPFMA DEBUG   :'',
     -      '' Elements left: '',I10,'', background discarded: '',
     -      I10,'', points: '',I10)') NTETRA,NDELET,NPOINT
...
*   Two possible formats for this file: NLIST,,,,COORD or NLIST
       IF(INDEX(STRING,'THXY    THYZ    THZX').NE.0)LCOORD=.FALSE.
...
*   Version 11.x and 12.x
            IF(STRING(9:9).EQ.' ')THEN
                 READ(STRING,'(I8,2X,3E20.12)',ERR=2010,IOSTAT=IOS)
     -                IP,XP,YP,ZP
*   Version 13.x, 14.x and 15.x
            ELSE
                 READ(STRING,'(BN,I10,3E21.14)',ERR=2010,IOSTAT=IOS)
     -                IP,XP,YP,ZP
            ENDIF
       ELSE
...
*   End of reading, make sure that all node references are solved.
       ...
       IF(EXMAP(I,J).GE.0)THEN
            PRINT *,' !!!!!! MAPFMA WARNING : Unresolved references'//
...
**  Print number of deleted tetrahedrons.
     ...
     -      '', nodes: '',I6)') NTETRA,NDELET,NPOINT
       IF(NDELET.NE.0)PRINT *,' ------ MAPFMA MESSAGE : Found ',NDELET,
...
*   Otherwise, close the points file and re-open mesh file.
       ELSE
            ...
            IF(IFAIL1.NE.0)THEN
                 PRINT *,' !!!!!! MAPFMA WARNING : Re-opening the'//
...
*** Check that we have got all materials fully defined.
       ...
       IF(EPSMAT(IEPS).LT.0)THEN
            PRINT *,' !!!!!! MAPFMA WARNING : Material ',IEPS,
...
2005   CONTINUE
       PRINT *,' !!!!!! MAPFMA WARNING : Premature end of file'//
     -      ' reading a field map file; map not available.'
       IF(LDEBUG)CALL INPIOS(IOS)
       CALL INPSWI('RESTORE')
       RETURN
2010   CONTINUE
       PRINT *,' !!!!!! MAPFMA WARNING : Error reading a field'//
     -      ' map file; map not available.'
       IF(LDEBUG)CALL INPIOS(IOS)
       CALL INPSWI('RESTORE')
       RETURN
2030   CONTINUE
       PRINT *,' !!!!!! MAPFMA WARNING : Error closing a field'//
     -      ' map file ; map not available.'
       IF(LDEBUG)CALL INPIOS(IOS)
       RETURN
2040   CONTINUE
       PRINT *,' !!!!!! MAPFMA WARNING : Error backspacing on a'//
