CDECK  ID>, GASG09.
       SUBROUTINE GASG09(IFAIL)
*-----------------------------------------------------------------------
*   GASG09 - This routine retrieves the gas data written to an external
...
*** Identify the routine.
       IF(LIDENT)PRINT *,' /// ROUTINE GASG09 ///'
...
*** Open a dataset and inform DSNLOG.
       ...
       IF(IFAIL1.NE.0)THEN
            PRINT *,' !!!!!! GASG09 WARNING : Opening ',FILE(1:NCFILE),
     -           ' failed ; gas data are not read.'
            RETURN
       ENDIF
       CALL DSNLOG(FILE,'Gas data  ','Sequential','Read only ')
       IF(LDEBUG)PRINT *,' ++++++ GASG09 DEBUG   : Dataset ',
...
*   Locate the pointer on the header of the requested member.
       ...
       IF(.NOT.EXIS)THEN
            ...
            IF(EXIS)THEN
                 PRINT *,' ###### GASG09 ERROR   : Gas description '//
...
*   Check that this member contains indeed gas data.
       ...
       IF(LDEBUG)THEN
            PRINT *,' ++++++ GASG09 DEBUG   : Dataset header'//
     -           ' record follows:'
            PRINT *,STRING
       ENDIF
...
*   Check the version.
       ...
       IF(STRING(1:14).EQ.' Version   : 8')THEN
            PRINT *,' ------ GASG09 MESSAGE : This member is'//
     -           ' written in earlier format 8.'
            CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
            CALL GASG08(IFAIL)
            RETURN
       ELSEIF(STRING(1:14).NE.' Version   : 9')THEN
            PRINT *,' !!!!!! GASG09 WARNING : This member'//
...
*   Heed initialisation data.
       CALL GASHGT(IFAIL1)
       IF(IFAIL1.NE.0)THEN
            PRINT *,' !!!!!! GASG09 WARNING : Reading Heed data'//
...
*   SRIM initialisation data.
       CALL GASSG8(IFAIL1)
       PRINT *,' !!!!!! GASG08 WARNING : SRIM data from version 9'//
...
2000   CONTINUE
       PRINT *,' ###### GASG09 ERROR   : EOF encountered while reading',
     -      ' '//FILE(1:NCFILE)//' from unit 12 ; no gas data read.'
       CALL INPIOS(IOS)
       CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
       RETURN
2010   CONTINUE
       PRINT *,' ###### GASG09 ERROR   : Error while reading'//
     -      ' '//FILE(1:NCFILE)//' from unit 12 ; no gas data read.'
       CALL INPIOS(IOS)
       CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
       RETURN
2030   CONTINUE
       PRINT *,' ###### GASG09 ERROR   : Dataset '//FILE(1:NCFILE)//
