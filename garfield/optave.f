CDECK  ID>, OPTAVE.
       SUBROUTINE OPTAVE(AVER,IFAIL)
*-----------------------------------------------------------------------
*   OPTAVE - Routine returning the current function average.
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      IPTECH,LREPSK,LKINK,LSTMAX,LEQSRT,LEQCRS,LEQMRK,LAVPRO
       REAL VAR(MXVAR),RES(1),QPLT,AVER,ANG,DRES,EZ
       INTEGER MODVAR(MXVAR),MODRES(1),ITYPE,IFAIL,I,J,NDATA,ILOC,
     -      IANG
*** Drift line parameters.
       QPLT=-1.0
       ITYPE=1
*** Preset the sum and the number of data-points.
       AVER=0.0
       NDATA=0
*** The variable modes never change.
       DO 5 I=1,9
       MODVAR(I)=2
5      CONTINUE
...
*** over the wires or ...
     ...
     -           REAL(NPOINT-1)
            VAR(1)=X(I)+COS(ANG)*DRES/2
            VAR(2)=Y(I)+SIN(ANG)*DRES/2
            CALL EFIELD(VAR(1),VAR(2),0.0,
     -           VAR(3),VAR(4),EZ,VAR(5),VAR(6),
     -           IOPT,ILOC)
            VAR(7)=0.0
...
60          CONTINUE
            D(I)=DRES
50          CONTINUE
*** Or over something unknown.
       ELSE
            PRINT *,' ###### OPTAVE ERROR   : Unknown averaging type ',
     -           PNTTYP,' received; program bug - please report.'
            RETURN
       ENDIF
*** Check there are enough data-points.
       IF(NDATA.LT.1)THEN
            PRINT *,' !!!!!! OPTAVE WARNING : Insufficient number of',
     -           ' normal data-points found.'
            IFAIL=1
            RETURN
       ENDIF
*** Calculate the average.
       AVER=AVER/REAL(NDATA)
*** Reset IFAIL to 0, things seem to be OK.
       IFAIL=0
       END
