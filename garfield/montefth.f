CDECK  ID>, MONTEFTH.
      SUBROUTINE MONTEFTH(IFAIL)
*-----------------------------------------------------------------------
*   MONTEFTH - Calculates collision events and updates diffusion and
*              velocity. This routine handles terminations at fixed
*              drift times. Solves motion in coordinate system with
*              bfield aligned to x-axis electric field at an angle
*              btheta in the x-z plane. The results for the velocity
*              vectors are then rotated into the standard coordinate
*              frame with the electric field along the z-axis and the
*              bfield at an angle BTHETA to the electric field in the
*              x-z plane.
...
*-----------------------------------------------------------------------
     ...
     -         LSYNCH,LINPRD,LUNOUT,JFAIL,JEXMEM
       DOUBLE PRECISION EPRM(4000000),DRAND48,S,RDUM,E1,E,EI,
     -      XTOT,XTOTS,YTOT,YTOTS,XSTRT,YSTRT,ZSTRT,TSSTRT,
     -      CONST9,ARG1,D,Q,U,CSQD,ARGZ,WX,WY,WZ,WXR,WYR,WZR,
     -      RTHETA,VTOT,TLIM,TDASH,TSTOP,T,T2,WBT,COSWT,SINWT,TTEM,
     -      DCX1,DCY1,DCZ1,CX1,CY1,CZ1,DCX100,DCY100,DCZ100,CX2,CY2,CZ2,
     -      DCX2,DCY2,DCZ2,E100,DX,DZ,ESEC,PHI0,THETA0,
     -      F3,F4,F5,F6,F8,F9,R1,R2,R3,R31,R4,R5,R9,S1,S2,
     -      ANEION,ANBT,EPRMBAR,E2PRM,EBAR,EERR
       INTEGER IESPECP(100),I,ID,INTEM,I100,NCOL,NELEC,NEION,NMXADD,
     -      NPONT,NCLUS,J1,JPRINT,IPRINT,ITER,IPLANE,IDUM,IE,IT,IPT,JCT,
     -      IFAIL
       EXTERNAL DRAND48
       logical lab20
*** Identify the routine if requested.
       IF(LIDENT)PRINT *,' /// ROUTINE MONTEFTH ///'
...
*   in this cycle if so output current results.
 ...
 944  FORMAT(/,2X,'PROGRAM STOPPED TOO MANY PRIMARIES IPRIM =',I7)
       PRINT *,' !!!!!! MONTEFTH WARNING: Attachment too high;'//
...
*   Store position and energy at time plane =IPLANE.
                CALL TPLANEH(T,E1,CX1,CY1,CZ1,EMAG,IPLANE)
...
C  20           continue
 ...
 913  FORMAT(3X,' AFTER STORE ITER=',I10,' DZ=',D12.3,'E1=',D12.3,
     -     ' COSWT=',D12.3,' SINWT=',D12.3,' WBT=',D12.3,' CY1=',D12.3)
      IF(E.LT.0.0D0) THEN
           IF(LBMCPR)WRITE(LUNOUT,913)ITER,DZ,E,COSWT,SINWT,WBT,CY1
           E=0.001D0
      ENDIF
      IE=INT(E/ESTEP)+1
      IE=MIN(IE,2048)
...
*** Rotate intermediate output into lab frame
      ...
      WYR=WY
      WXR=WZ*RSN+WX*RCS
C     IF(LBMCPR)WRITE(LUNOUT,202) WZR,WYR,WXR,TTOTS,JCT
