CDECK  ID>, FFCD10.
       SUBROUTINE FFCD10(XPOS,YPOS,EX,EY)
*-----------------------------------------------------------------------
...
*   First the case that the wire has to be taken fully.
       IF(CNALSO(I))THEN
            EX=EX+E(I)*REAL(1/CONJG(ZPOS-ZI)+
     -           ZI/(COTUBE**2-CONJG(ZPOS)*ZI))
            EY=EY+E(I)*AIMAG(1/CONJG(ZPOS-ZI)+
     -           ZI/(COTUBE**2-CONJG(ZPOS)*ZI))
*   Otherwise only take the mirror charge.
       ELSE
            EX=EX+E(I)*REAL(ZI/(COTUBE**2-CONJG(ZPOS)*ZI))
            EY=EY+E(I)*AIMAG(ZI/(COTUBE**2-CONJG(ZPOS)*ZI))
       ENDIF
