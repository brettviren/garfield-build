CDECK  ID>, DLCFMP.
       SUBROUTINE DLCFMP(XX0,YY0,ZZ0,XX1,YY1,ZZ1,DT,ILOC,Q,ITYPE)
*-----------------------------------------------------------------------
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -         LSYNCH,LINPRD,LUNOUT,JFAIL,JEXMEM
       ...
       REAL Q,EX,EY,EZ,ETOT,VOLT,EOVERM
       DOUBLE PRECISION XX0,YY0,ZZ0,XX1,YY1,ZZ1,X0,Y0,Z0,X1,Y1,Z1,
     -      XM,YM,ZM,POS(3),FF0(3),F0(3),ACC(3),SPEED,ACCEL,STEP,
     -      XOLD,YOLD,ZOLD,DT
       COMMON /VFUCOM/ EOVERM,ILOCVF
*** Identify this routine if requested.
       IF(LIDENT)PRINT *,' /// ROUTINE DLCFMP ///'
*** Ensure there is a previous stored step.
       IF(NU.LE.0)THEN
            PRINT *,' ###### DLCFMP ERROR   : Called at first step;'//
...
*** Check we may still add points.
       IF(NU.GE.MXLIST)THEN
            ISTAT=-2
            IF(LDEBUG)PRINT *,' ++++++ DLCFMP DEBUG   : Last point'//
     -           ' not added because MXLIST is reached.'
            RETURN
       ENDIF
*** Ensure we got an appropriate location code.
       IF(ILOC.NE.-5.AND.ILOC.NE.-6)THEN
            PRINT *,' ###### DLCFMP ERROR   : Called for location'//
...
*** Initialise the bisection loop.
       ...
       Y1=YY1
       Z1=ZZ1
       CALL EFIELD(REAL(X0),REAL(Y0),REAL(Z0),EX,EY,EZ,ETOT,VOLT,0,
     -      ILOC0)
       CALL EFIELD(REAL(X1),REAL(Y1),REAL(Z1),EX,EY,EZ,ETOT,VOLT,0,
     -      ILOC1)
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ DLCFMP DEBUG   : Starting'',
     -      '' from (x,y,z)='',3E15.8,'' loc='',I5/34X,
     -      '' to   (x,y,z)='',3E15.8,'' loc='',I5)') X0,Y0,Z0,ILOC0,
     -      X1,Y1,Z1,ILOC1
       IF(ILOC0.NE.0.OR.ILOC1.EQ.0)THEN
            IF(LDEBUG)PRINT *,' ++++++ DLCFMP DEBUG   : Called but'//
...
*   Quit bisection when interval becomes too small.
     ...
     -      ABS(Z1-Z0).LE.1D-6*(ABS(Z0)+ABS(Z1)))THEN
            IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ DLCFMP DEBUG   :'',
...
*   Middle point.
       ...
       ZM=0.5*(Z0+Z1)
*   Evaluate field.
       CALL EFIELD(REAL(XM),REAL(YM),REAL(ZM),EX,EY,EZ,ETOT,VOLT,0,
     -      ILOCM)
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ DLCFMP DEBUG   : Bisection'',
...
*   Compute drift velocity at begin and end of the step.
            ...
            CALL DLCVEL(X0,Y0,Z0,F0,Q,ITYPE,ILOC1)
...
*   Or approximate with the last step only.
            ELSEIF(ILOC0.EQ.0)THEN
                 ...
                 PRINT *,' ------ DLCFMP MESSAGE : Unable to compute'//
     -                ' mean drift speed at last step; approximated.'
*   At least one should be OK.
            ELSE
                 PRINT *,' !!!!!! DLCFMP WARNING : Unable to compute'//
...
*   If there are already steps, estimate speed from the last step.
            IF(NU.GT.1)THEN
                 ...
                 ELSE
                      PRINT *,' !!!!!! DLCFMP WARNING : Drift speed'//
...
**  Microscopic tracking
       ELSEIF(ITYPE.EQ.4)THEN
            IF(DT.LE.0)THEN
                 PRINT *,' !!!!!! DLCFMP WARNING : Time delay over'//
...
**  Anything else
       ELSE
            PRINT *,' ###### DLCFMP ERROR   : Unknown tracking method'//
...
**  Check velocity.
       IF(SPEED.LE.0)THEN
            PRINT *,' !!!!!! DLCFMP WARNING : Drift line not properly'//
...
*** Check that the particle is still inside the drift area, clip if not.
     ...
     -                 (ZOLD-ZU(NU-1))**2))
            ENDIF
            IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ DLCFMP DEBUG   : Area'',
     -           '' left or solid entered, ISTAT='',I5)') ISTAT
       ENDIF
*** Debugging output.
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ DLCFMP DEBUG   : NU='',I5/
