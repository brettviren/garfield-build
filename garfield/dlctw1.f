...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      EX,EY,EZ,ETOT,BX,BY,BZ,BTOT,VOLT,GASTWN,DRES,
     -      BXM,BYM,BZM,BTOTM
...
*** Identify the routine
       IF(LIDENT)PRINT *,' /// ROUTINE DLCTW1 ///'
...
*   Cheat in case the point is located inside a wire.
     ...
     -           BX,BY,BZ,BTOT)
            D(ILOCRS)=DRES
            IF(LDEBUG)PRINT *,' ++++++ DLCTW1 DEBUG   : Drift-line',
...
*   In case this didn't help, just log the failure.
     ...
     -           (YU(IU)-YU(IU-1))**2+(ZU(IU)-ZU(IU-1))**2)
       ENDIF
       IF(IU.GT.1)CRUDE=CRUDE+STEP*(TWNVEC(IU)+TWNVEC(IU-1))/2.0
100    CONTINUE
       NFC=NU
*** Print a heading for the debugging output.
       IF(LDEBUG)THEN
            PRINT *,' ++++++ DLCTW1 DEBUG   : Townsend integration',
...
*   Cheat in case the point is located inside a wire.
     ...
     -           BXM,BYM,BZM,BTOTM)
            NFC=NFC+1
            D(ILOCRS)=DRES
            IF(LDEBUG)PRINT *,' ++++++ DLCTW1 DEBUG   : Intermediate',
...
*** Finally take the exponential.
       ...
       ELSEIF(ALFINT.LT.46.0)THEN
            FACTOR=EXP(ALFINT)
       ELSE
            PRINT *,' !!!!!! DLCTW1 WARNING : The Townsend coefficient',
     -           ' can not be integrated without overflow; set to 1E20.'
            FACTOR=1.0E20
       ENDIF
       IF(LDEBUG)THEN
            PRINT *,' ++++++ DLCTW1 DEBUG   : EFIELD calls: ',NFC,
