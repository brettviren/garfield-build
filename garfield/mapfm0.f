CDECK  ID>, MAPFM0.
       SUBROUTINE MAPFM0(FMAP,NCMAP,IDATA,IWMAP,DELBKG,IFAIL)
*-----------------------------------------------------------------------
*   MAPFM0 - Reads a Maxwell 3D Version 11, 2nd order tetrahedrons.
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      IDATA,NWORD,NCAUX,NCSHD,IEND,IP,NTETRA,NTOTAL,NPOINT,
     -      MTETRA,LOOKUP(10),NDELET,INEAR
C       INTEGER MPOINT
       REAL TEMP(10),SUM,ECOMP,DCOMP,DX,DY,DZ,XP,YP,ZP,DNEAR
       CHARACTER*(*) FMAP
       CHARACTER*(MXNAME) FAUX,FSHD
       CHARACTER*80 STRING
       LOGICAL SCALAR,READ,NEWEPS,EXIST,EXIST2,DELBKG,
     -      DELFLG(MXMAP),FIRST
       EXTERNAL INPCMP
       SAVE NTETRA,NPOINT,LOOKUP,NTOTAL
       DATA NTETRA/0/, NPOINT/0/
*** Lookup table from Daniel Barna.
       DATA LOOKUP/1, 6, 3, 5, 8, 2, 7, 10, 9, 4/
**** Identify the routine if requested.
       IF(LIDENT)PRINT *,' /// ROUTINE MAPFM0 ///'
*** Assume that this will fail.
       IFAIL=1
*** Make sure the file names are not too long.
       IF(NCMAP.GT.MXNAME)THEN
            PRINT *,' !!!!!! MAPFM0 WARNING : Field map file name ',
     -           FMAP(1:NCMAP),' is too long; not read.'
            CALL INPSWI('RESTORE')
            RETURN
       ENDIF
*** Check the delete background option, mandatory for this format.
       IF(.NOT.DELBKG)THEN
            PRINT *,' ------ MAPFM0 MESSAGE : The DELETE-BACKGROUND'//
     -           ' option is mandatory for Maxwell-11; switched on.'
            DELBKG=.TRUE.
       ENDIF
...
1050        CONTINUE
            IF(IEND.EQ.0)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : Mesh file name'//
     -                ' empty ; not read.'
                 CALL INPSWI('RESTORE')
                 RETURN
            ENDIF
...
*   Check for the existence of the files.
            ...
            IF(.NOT.EXIST)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : Hydra file '//
     -                FAUX(1:NCAUX)//'hyd not found; map not read.'
            ...
            IF(.NOT.EXIST)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : Point file '//
     -                FAUX(1:NCAUX)//'pnt not found; map not read.'
                 CALL INPSWI('RESTORE')
                 RETURN
            ENDIF
            IF(DELBKG)THEN
                 CALL DSNINQ(FSHD(1:NCSHD),NCSHD,EXIST)
                 IF(.NOT.EXIST)THEN
                      PRINT *,' !!!!!! MAPFM0 WARNING : Solid file '//
     -                     FSHD(1:NCSHD)//' not found; map not read.'
                      CALL INPSWI('RESTORE')
                      RETURN
                 ENDIF
            ENDIF
...
1070        CONTINUE
            IF(IEND.EQ.0)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : Field file name'//
     -                ' empty ; not read.'
                 CALL INPSWI('RESTORE')
                 RETURN
            ENDIF
            ...
            NCAUX=2
1080        CONTINUE
*   Create a solids file name.
            FSHD=FAUX(1:NCAUX)//'fields.shd'
            NCSHD=NCAUX+10
*   And check its existence
            IF(DELBKG)THEN
                 CALL DSNINQ(FSHD(1:NCSHD),NCSHD,EXIST2)
            ELSE
                 EXIST2=.TRUE.
            ENDIF
*   Test for various files.
     ...
     -                EXIST)
                 IF(EXIST.AND.EXIST2)THEN
                      ...
                      NCAUX=NCAUX+9
                      PRINT *,' ------ MAPFM0 MESSAGE : Taking the'//
     -                     ' "fileset2" mesh.'
                      GOTO 1090
                 ENDIF
            ENDIF
     ...
     -                     ' "initial" mesh.'
                 ...
                 ENDIF
            ENDIF
            PRINT *,' !!!!!! MAPFM0 WARNING : Hydra, point and'//
...
1150   CONTINUE
       IF(DELBKG)THEN
*   Open the solids file.
            CALL DSNOPN(FSHD,NCSHD,12,'READ-FILE',IFAIL1)
            IF(IFAIL1.NE.0)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : Unable to open the'//
     -                ' solid file '//FSHD(1:NCSHD)//'; map not read.'
                 RETURN
            ENDIF
*   Record the opening.
            CALL DSNLOG(FSHD(1:NCSHD),'Solids    ','Sequential',
     -           'Read only ')
...
*   Check for empty files.
            IF(NWORD.EQ.0)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : The file '//
     -                FSHD(1:NCSHD)//' seems to be empty; map not read.'
                 CALL INPSWI('RESTORE')
...
*   Read the number of tetrahedrons.
            ...
            IF(IFAIL1.NE.0.OR.NTOTAL.LE.0.OR.NWORD.NE.2)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : The file '//
     -                FSHD(1:NCSHD)//' has an unreadable number'//
     -                ' of tetrahedrons; not read.'
                 CALL INPSWI('RESTORE')
                 RETURN
            ELSEIF(NTOTAL.GT.MXMAP)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : Number of'//
     -                ' tetrahedrons in '//FSHD(1:NCSHD)//
     -                ' exceeds compilation limit; file not read.'
                 CALL INPSWI('RESTORE')
                 RETURN
            ENDIF
*   Debugging output.
            IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ MAPFM0 DEBUG   :'',
     -           '' Number of .shd tetrahedrons (incl bkg): '',I6)')
     -           NTOTAL
...
*   Read the number of tetrahedrons.
     ...
     -           ' exceeds compilation limit; file not read.'
       ...
       ENDIF
*   Debugging output.
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ MAPFM0 DEBUG   : Number'',
     -      '' of .hyd tetrahedrons (incl bkg): '',I6)') NTOTAL
**  Loop over the tetrahedrons, with progress printing.
...
*   Read the data line.
       ...
       CALL INPNUM(NWORD)
       IF(NWORD.NE.6)THEN
            PRINT *,' !!!!!! MAPFM0 WARNING : The format of '//
...
*   Find the pointers to the .pnt file.
     ...
     -      ITETRA.LE.0.OR.ITETRA.GT.NTOTAL)THEN
            PRINT *,' !!!!!! MAPFM0 WARNING : Reference to points'//
...
*   Make sure we're at the end.
       ...
       IF(STRING(1:9).NE.'end_hydra')PRINT *,' !!!!!! MAPFM0 WARNING'//
...
*   Read the point coordinates and the reference to the .hyd file.
     ...
     -      IP.LE.0)THEN
            PRINT *,' !!!!!! MAPFM0 WARNING : Reference to hydra'//
     -           ' unreadable in '//FAUX(1:NCAUX)//'; map not read.'
            CALL INPSWI('RESTORE')
            RETURN
       ENDIF
...
*   Make sure we're at the end.
       ...
       IF(STRING(1:11).NE.'end_points')PRINT *,' !!!!!! MAPFM0'//
...
*   End of reading, make sure that all hydra references are solved.
       ...
       IF(EXMAP(I,J).GE.0)THEN
            PRINT *,' !!!!!! MAPFM0 WARNING : Unresolved references'//
...
**  Print number of deletec tetrahedrons.
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ MAPFM0 DEBUG   :'',
     -      '' Tetrahedrons: '',I6,'' Background: '',I6)')
     -      NTETRA,NDELET
       IF(NDELET.NE.0)PRINT *,' ------ MAPFM0 MESSAGE : Found ',NDELET,
...
*   Otherwise, close the points file and re-open mesh file.
       ELSE
            ...
            IF(IFAIL1.NE.0)THEN
                 PRINT *,' !!!!!! MAPFM0 WARNING : Re-opening the'//
...
*   Potentials.
     ...
     -                '' File contains a potential.'')')
                 IF(MAPFLG(5))PRINT *,' ------ MAPFM0 MESSAGE :'//
...
*   Dielectric constants.
     ...
     -                '' File contains an epsilon map.'')')
                 IF(MAPFLG(9))PRINT *,' ------ MAPFM0 MESSAGE :'//
...
**  Ensure that the data type matches the declared type.
     ...
     -      (IDATA.NE.0.AND.IDATA.NE.9)))THEN
            PRINT *,' !!!!!! MAPFM0 WARNING : Field ',I,' of file ',
...
*   Ensure this number is in range.
       IF(ITETRA.LE.0.OR.ITETRA.GT.NTOTAL)THEN
            PRINT *,' !!!!!! MAPFM0 WARNING : Tetrahedron number ',
...
80               CONTINUE
                 IEPS=-1
                 IF(INEAR.GT.0)THEN
                      IF(DNEAR.LT.1E-3*(ABS(SUM)+ABS(EPSMAT(INEAR))))
     -                     IEPS=INEAR
                 ENDIF
                 IF(IEPS.LT.0.AND.NEPS.LT.MXEPS)THEN
                      ...
                      IEPS=NEPS
                      EPSMAT(IEPS)=SUM
                      IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ MAPFM0'',
     -                     '' DEBUG   : Adding dielectricum with'',
     -                     '' eps='',E12.5,''.'')') EPSMAT(IEPS)
                 ELSEIF(IEPS.LT.0.AND.INEAR.GT.0)THEN
                      IF(JFAIL.EQ.1)THEN
                           PRINT *,' !!!!!! MAPFM0 WARNING :'//
...
*   D field.
     ...
     -                (TEMP(K),K=1,3)
                 IF(LOOKUP(J).LE.4)THEN
                      DX=DX+TEMP(1)/(10000*EPS0)
                      DY=DY+TEMP(2)/(10000*EPS0)
                      DZ=DZ+TEMP(3)/(10000*EPS0)
                 ENDIF
50               CONTINUE
                 DCOMP=SQRT(DX**2+DY**2+DZ**2)/4
            ENDIF
**  Dielectricum identification via D/E comparison.
     ...
     -           ICONT(3).EQ.4))THEN
                 IEPS=-1
                 ECOMP=SQRT(((EXMAP(NTETRA,1)+EXMAP(NTETRA,2)+
     -                EXMAP(NTETRA,3)+EXMAP(NTETRA,4))**2+
     -                (EYMAP(NTETRA,1)+EYMAP(NTETRA,2)+
     -                EYMAP(NTETRA,3)+EYMAP(NTETRA,4))**2+
     -                (EZMAP(NTETRA,1)+EZMAP(NTETRA,2)+
     -                EZMAP(NTETRA,3)+EZMAP(NTETRA,4))**2))/4
                 DO 60 J=1,NEPS
                 IF(ABS(ECOMP*EPSMAT(J)-DCOMP).LE.1E-3*
     -                (ABS(ECOMP)+ABS(DCOMP)))IEPS=J
60               CONTINUE
                 IF(ECOMP.LE.0.AND.DCOMP.GT.0)THEN
                      PRINT *,' !!!!!! MAPFM0 WARNING : E = 0 but D = ',
     -                     DCOMP,' V/cm in tetrahedron ',NTETRA,
     -                     ' can not determine epsilon; skipped.'
                 ELSEIF(IEPS.LT.0.AND.NEPS.GE.MXEPS)THEN
                      IF(NEPS.GE.1)THEN
                           DNEAR=ABS(EPSMAT(1)-DCOMP/ECOMP)
                           IEPS=1
                      ELSE
                           DNEAR=0
                           IEPS=-1
                      ENDIF
                      DO 61 J=2,NEPS
                      IF(ABS(EPSMAT(J)-DCOMP/ECOMP).LT.DNEAR)THEN
                           IEPS=J
                      ENDIF
61                    CONTINUE
     ...
     -                     '' eps='',E12.5,''.'')') EPSMAT(IEPS)
                 ...
                 MATMAP(NTETRA)=IEPS
                 NEWEPS=.TRUE.
*  Otherwise store the field (compensate for unit scaling).
            ELSEIF(ICONT(1).EQ.-9.AND.ICONT(2).EQ.-9.AND.
     -           ICONT(3).EQ.-9.AND.(.NOT.MAPFLG(2)))THEN
                 EXMAP(NTETRA,1)=DCOMP*100
            ENDIF
       ENDIF
10     CONTINUE
...
2000   CONTINUE
     ...
     -           ' match current mesh; not read.'
            PRINT *,'                         From mesh: ',NMAP,
     -           ', found in field map: ',NTETRA
            RETURN
       ENDIF
...
2005   CONTINUE
       PRINT *,' !!!!!! MAPFM0 WARNING : Premature end of file'//
     -      ' reading a mesh file; map not available.'
       IF(LDEBUG)CALL INPIOS(IOS)
       CLOSE(12,ERR=2030)
       RETURN
2010   CONTINUE
       PRINT *,' !!!!!! MAPFM0 WARNING : Error reading field map'//
     -      ' file ',FMAP(1:NCMAP),'; map not available.'
       IF(LDEBUG)CALL INPIOS(IOS)
       RETURN
2015   CONTINUE
       PRINT *,' !!!!!! MAPFM0 WARNING : Error reading a mesh'//
     -      ' file ; map not available.'
       IF(LDEBUG)CALL INPIOS(IOS)
       CLOSE(12,ERR=2030)
       RETURN
2030   CONTINUE
       PRINT *,' !!!!!! MAPFM0 WARNING : Error closing field map'//
     -      ' file ',FMAP(1:NCMAP),'; map not available.'
       IF(LDEBUG)CALL INPIOS(IOS)
       RETURN
       END
