CDECK  ID>, ALPCLCA.
       SUBROUTINE ALPCLCA(IFAIL)
*-----------------------------------------------------------------------
*   ALPCLCA - Estimate time step for avalanche simulation in time of
*             flight sim. Uses estimated gain of 3.0 between planes.
*             Calls TOF and PT subroutines and updates ALPHA and ATT.
*             Version with B-field parallel to E-field.
*-----------------------------------------------------------------------
       implicit none
...
*-----------------------------------------------------------------------
     ...
     -         LSYNCH,LINPRD,LUNOUT,JFAIL,JEXMEM
       INTEGER IMAX,IFAIL
       DOUBLE PRECISION WRN,FC1,FC2,ALPZZ,ALPATT,RATIO
*** Assume that this will work.
       IFAIL=0
*** Increase NMAX if needed.
      IMAX=NMAX/10000000
      IF(IMAX.LT.5) IMAX=5
      NMAX=IMAX*10000000
...
*** Calc time of flight and pt
   ...
   26 FORMAT(1(/),'  TIME STEP BETWEEN SAMPLING PLANES =',D12.5,' PICOSE
     /CS.',/)
      CALL MONTEFTA(IFAIL)
      IF(IFAIL.NE.0)THEN
           PRINT *,' !!!!!! ALPCLCA WARNING : Failure in MONTEFTA;'//
