CDECK  ID>, DLCAT2.
       SUBROUTINE DLCAT2(FACTOR)
*-----------------------------------------------------------------------
*   DLCAT2 - Routine returning the attachment losses for the current
*            drift line. The routine uses an adaptive Simpson style
*            integration.
*   VARIABLES : BETA.      : Attachment coefficients (1,2 end; M middle)
*               BETINT     : Integral of the attachment coefficient.
*               FACTOR     : The attachment losses
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -         LSYNCH,LINPRD,LUNOUT,JFAIL,JEXMEM
       REAL ATTVEC(MXLIST),BETA1,BETA2,BETAM,FACTOR,EXM,EYM,EZM,ETOTM,
     -      EX,EY,EZ,ETOT,VOLT,GASATT,DRES,SCALE,BX,BY,BZ,BTOT,
     -      BXM,BYM,BZM,BTOTM
       DOUBLE PRECISION XAUX1,XAUX2,YAUX1,YAUX2,STEP,BETINT,
...
*** Identify the routine
       IF(LIDENT)PRINT *,' /// ROUTINE DLCAT2 ///'
...
*   Cheat in case the point is located inside a wire.
     ...
     -           BX,BY,BZ,BTOT)
            D(ILOCRS)=DRES
            IF(LDEBUG)PRINT *,' ++++++ DLCAT2 DEBUG   : Drift-line',
...
C      print *,' z: ',zu(iu-1),zu(iu),vd(3)
       ENDIF
*   Compute attachment coefficients and step length.
       IF(POLAR)THEN
            ATTVEC(IU)=GASATT(EX/EXP(REAL(XU(IU))),EY/EXP(REAL(XU(IU))),
     -           EZ,BX,BY,BZ)
            IF(IU.GT.1)THEN
     ...
     -                (ZU(IU)-ZU(IU-1))**2)
            ENDIF
       ELSE
            ATTVEC(IU)=GASATT(EX,EY,EZ,BX,BY,BZ)
            IF(IU.GT.1)STEP=SQRT((XU(IU)-XU(IU-1))**2+
     -           (YU(IU)-YU(IU-1))**2+(ZU(IU)-ZU(IU-1))**2)
       ENDIF
       IF(IU.GT.1)CRUDE=CRUDE+STEP*SCALE*(ATTVEC(IU)+ATTVEC(IU-1))/2.0
100    CONTINUE
       NFC=NU
*** Ensure that the crude sum is positive.
       IF(CRUDE.LT.0)THEN
            PRINT *,' !!!!!! DLCAT2 WARNING : Negative attachment sum'//
...
*** Print a heading for the debugging output.
       IF(LDEBUG)THEN
            PRINT *,' ++++++ DLCAT2 DEBUG   : Attachment integration',
...
*   Compute the field and the attachment coeff. at the middle point.
...
*   Add the new term to the integral.
            BETINT=BETINT+STEP*SCALE*(BETA1+4.0*BETAM+BETA2)/6.0
...
*   Print some debugging output.
     ...
     -      YU(IU+1),ZU(IU+1),EXP(-MIN(46.0D0,BETINT))
10     CONTINUE
*** Finally take the exponential.
       IF(BETINT.LT.0.0.OR.BETINT.GT.46.0)THEN
            FACTOR=1.0
       ELSE
            FACTOR=EXP(-BETINT)
       ENDIF
       IF(LDEBUG)THEN
            PRINT *,' ++++++ DLCAT2 DEBUG   : EFIELD calls: ',NFC,
     -           ', deepest stack: ',ISTACK
            PRINT *,'                         Final log estimate: ',
     -           BETINT,' (crude estimate: ',CRUDE,').'
       ENDIF
       END
