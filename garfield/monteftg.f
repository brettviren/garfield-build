CDECK  ID>, MONTEFTG.
      SUBROUTINE MONTEFTG(IFAIL)
*-----------------------------------------------------------------------
*   MONTEFTG - Calculates collision events and updates diffusion and
*             velocity. This routine handles terminations at fixed
*             drift times. B field at 90 degrees to efield.
...
*-----------------------------------------------------------------------
     ...
     -         LSYNCH,LINPRD,LUNOUT,JFAIL,JEXMEM
       DOUBLE PRECISION EPRM(4000000),DRAND48,S,YTOT,YTOTS,RDUM,E1,
     -      CONST9,YSTRT,ZSTRT,TSSTRT,DCX1,DCY1,DCZ1,VTOT,TLIM,TDASH,
     -      TSTOP,T,WBT,COSWT,SINWT,DZ,E,TTEM,T2,DCX2,DCY2,DCZ2,
     -      CX1,CY1,CZ1,CX2,CY2,CZ2,E100,DCX100,DCY100,DCZ100,EI,ESEC,
     -      F3,F4,F5,F6,F8,F9,R1,R2,R3,R4,R5,R9,R31,S1,S2,THETA0,PHI0,
     -      ARG1,D,Q,U,CSQD,ARGZ,W,WY,ANEION,ANBT,EPRMBAR,E2PRM,EBAR,
     -      EERR
       INTEGER IESPECP(100),I,ID,INTEM,I100,NCOL,NELEC,NEION,NMXADD,
     -      NPONT,NCLUS,J1,IPRINT,JPRINT,ITER,IPLANE,IDUM,IE,IT,IPT,
     -      JCT,IFAIL
       logical lab20
       EXTERNAL DRAND48
*** Identify the routine if desired.
       IF(LIDENT)PRINT *,' /// ROUTINE MONTEFTG ///'
...
*   in this cycle if so output current results.
 ...
 944      FORMAT(/,2X,'PROGRAM STOPPED TOO MANY PRIMARIES IPRIM =',I7)
           PRINT *,' !!!!!! MONTEFTG WARNING: Attachment too high;'//
...
*    Store position and energy at time plane = IPLANE.
                CALL TPLANEG(T,E1,CX1,CY1,CZ1,EMAG,IPLANE)
...
C994  FORMAT(2X,' WARNING ENERGY =',F8.3,' LESS THAN ENERGY LOSS EI=',
C    -     F8.3,' AT ITER=',I12,' DUE TO BINNING ERROR')
...
C 201 FORMAT(/,7X,'INTERMEDIATE OUTPUT',/,
C    -     '    VELZ     POSZ       TIME',
C    -     '      VELY        COUNT    ')
C     IF(LBMCPR)WRITE(LUNOUT,202) W,ZTOTS,TTOTS,WY,JCT
...
C    -     /,10(2X,10I5,/))
 ...
 991        FORMAT(2(/),' PROGRAM STOPPED.  ITER =',I10,'    NMAX =',
     -           I10/' NPONT=',I4,' NELEC=',I8,' IPRIM=',I4,
     -           ' NMXADD=',I3)
            IFAIL=1
            RETURN
C            STOP
