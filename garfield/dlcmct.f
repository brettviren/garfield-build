CDECK  ID>, DLCMCT.
       SUBROUTINE DLCMCT(NE,NI)
*-----------------------------------------------------------------------
*   DLCMCT - Generates a random avalanche development.
*   VARIABLES :
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      BOLTZ=1.380658E-23)
       INTEGER NE,NI,NEW,ILOC,NINTER,I,J,K,MXVEC,IVECU,IVECN
     ...
     -      PROBTH,RVECU(MXVEC),RVECN(MXVEC),PALPHA,PETA,BX,BY,BZ,BTOT
       DOUBLE PRECISION VD(3),VTERM(3),WG6(6),TG6(6)
       LOGICAL TRY1,TRY2,DONE
       PARAMETER(PROBTH=0.01)
       LOGICAL LTOWN,LATTA
       EXTERNAL GASTWN,GASATT
       SAVE IVECU,IVECN,RVECU,RVECN,TG6,WG6
...
*** Identify the routine if requested.
       IF(LIDENT)PRINT *,' /// ROUTINE DLCMCT ///'
*** Initial number of electrons.
       NE=1
       NI=0
*** Make sure that electron drift velocities are available.
       IF(.NOT.(GASOK(1).AND.GASOK(4)))THEN
            PRINT *,' !!!!!! DLCMCT WARNING : Electron drift velocity'//
     -           ' data / avalanche data missing; no avalanche.'
            RETURN
       ENDIF
*   Establish the flags.
       LTOWN=.FALSE.
...
*** Check that a drift line exists.
       IF(IPTYPE.NE.1.OR.NU.LT.2)THEN
            PRINT *,' !!!!!! DLCMCT WARNING : Current drift line is'//
     -           ' not for an electron or too short; no avalanche.'
            RETURN
       ENDIF
*** Debugging output.
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ DLCMCT DEBUG   : '',
     -      26X,''Townsend:                     '',9X,L1/
     -      26X,''Attachment:                   '',9X,L1/
     -      26X,''Projected path length only:   '',9X,L1/
     -      26X,''Threshold for step divisions: '',F10.3/
     -      26X,''Number of steps:              '',5X,I5)')
     -      LTOWN,LATTA,LAVPRO,PROBTH,NU
...
*   See whether we succeeded.
            IF(.NOT.DONE)THEN
                 PRINT *,' !!!!!! DLCMCT WARNING : Unable to even out'//
...
*** Loop over the drift line.
       IF(LDEBUG)WRITE(LUNOUT,'(''  ++++++ DLCMCT DEBUG   : '',
     -      ''alpha [1/cm]    eta [1/cm]   electrons        ions'')')
       DO 10 I=1,NU-1
*   Compute the number of subdivisions.
       NINTER=(ALPHA(I)+ETA(I))/PROBTH
       IF(NINTER.LT.1)NINTER=1
       IF(LDEBUG)WRITE(LUNOUT,'(26X,2(F12.5,2X),2(I10,2X))')
     -      ALPHA(I),ETA(I),NE,NI
...
*   Gaussian approximation.
       IF(NE.GT.100)THEN
            ...
            ENDIF
            IF(LTOWN)THEN
                 NE=NE+NINT(REAL(NE)*PALPHA+RVECN(IVECN)*
     -                SQRT(REAL(NE)*PALPHA*(1-PALPHA)))
                 NI=NI+NINT(REAL(NE)*PALPHA+RVECN(IVECN)*
     -                SQRT(REAL(NE)*PALPHA*(1-PALPHA)))
                 IVECN=IVECN+1
            ENDIF
            IF(LATTA)THEN
                 NE=NE-NINT(REAL(NE)*PETA+RVECN(IVECN)*
     -                SQRT(REAL(NE)*PETA*(1-PETA)))
                 IVECN=IVECN+1
            ENDIF
