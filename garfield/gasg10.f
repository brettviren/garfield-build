CDECK  ID>, GASG10.
       SUBROUTINE GASG10(IFAIL)
*-----------------------------------------------------------------------
*   GASG10 - This routine retrieves the gas data written to an external
...
*   (Last changed on  2/ 8/10. Renamed on 25/ 8/09.)
*-----------------------------------------------------------------------
       implicit none
...
*** Identify the routine.
       IF(LIDENT)PRINT *,' /// ROUTINE GASG10 ///'
...
*** Open a dataset and inform DSNLOG.
       ...
       IF(IFAIL1.NE.0)THEN
            PRINT *,' !!!!!! GASG10 WARNING : Opening ',FILE(1:NCFILE),
     -           ' failed ; gas data are not read.'
            RETURN
       ENDIF
       CALL DSNLOG(FILE,'Gas data  ','Sequential','Read only ')
       IF(LDEBUG)PRINT *,' ++++++ GASG10 DEBUG   : Dataset ',
...
*   Locate the pointer on the header of the requested member.
       ...
       IF(.NOT.EXIS)THEN
            ...
            IF(EXIS)THEN
                 PRINT *,' ###### GASG10 ERROR   : Gas description '//
...
*   Check that this member contains indeed gas data.
       ...
       IF(LDEBUG)THEN
            PRINT *,' ++++++ GASG10 DEBUG   : Dataset header'//
     -           ' record follows:'
            PRINT *,STRING
       ENDIF
...
*   Check the version.
       ...
       IF(STRING(1:14).EQ.' Version   : 8')THEN
            PRINT *,' ------ GASG10 MESSAGE : This member is'//
     ...
     -           ' written in earlier format 9.'
            ...
            CALL GASG09(IFAIL)
            RETURN
       ELSEIF(STRING(1:15).NE.' Version   : 10')THEN
            PRINT *,' !!!!!! GASG10 WARNING : This member'//
...
*   Heed initialisation data.
       CALL GASHGT(IFAIL1)
       IF(IFAIL1.NE.0)THEN
            PRINT *,' !!!!!! GASG10 WARNING : Reading Heed data'//
...
*   SRIM initialisation data.
       CALL GASSG8(IFAIL1)
       PRINT *,' !!!!!! GASG08 WARNING : SRIM data from version 10'//
...
2000   CONTINUE
       PRINT *,' ###### GASG10 ERROR   : EOF encountered while reading',
     -      ' '//FILE(1:NCFILE)//' from unit 12 ; no gas data read.'
       CALL INPIOS(IOS)
       CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
       RETURN
2010   CONTINUE
       PRINT *,' ###### GASG10 ERROR   : Error while reading'//
     -      ' '//FILE(1:NCFILE)//' from unit 12 ; no gas data read.'
       CALL INPIOS(IOS)
       CLOSE(UNIT=12,STATUS='KEEP',IOSTAT=IOS,ERR=2030)
       RETURN
2030   CONTINUE
       PRINT *,' ###### GASG10 ERROR   : Dataset '//FILE(1:NCFILE)//
