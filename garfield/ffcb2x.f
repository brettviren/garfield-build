CDECK  ID>, FFCB2X.
       SUBROUTINE FFCB2X(XPOS,YPOS,EX,EY)
*-----------------------------------------------------------------------
...
*** Loop over all wires.
       ...
       XXNEG=(0.5*PI/SX)*(XPOS+X(I)-2*COPLAX)
       ZZ=CMPLX(XX,YY)
       ZZNEG=CMPLX(XXNEG,YY)
*** Calculate the field in case there are no equipotential planes.
       IF(CNALSO(I).AND.ABS(YY).LE.20)THEN
            ECOMPL=-B2SIN(I)/(SIN(ZZ)*SIN(ZZNEG))
       ELSEIF(ABS(YY).LE.20)THEN
            ECOMPL=-ICONS*(EXP(2*ICONS*ZZNEG)+1)/(EXP(2*ICONS*ZZNEG)-1)
       ELSE
            ECOMPL=0
       ENDIF
*** Take care of a planes at constant y.
       IF(YNPLAY)THEN
            YYMIRR=(0.5*PI/SX)*(YPOS+Y(I)-2*COPLAY)
