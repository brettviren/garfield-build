CDECK  ID>, DLCPHI.
       SUBROUTINE DLCPHI(PHI)
*-----------------------------------------------------------------------
*   DLCPHI - Computes the incidence angle of a drift line on an
*            electrode.
*   VARIABLES : PHI         - Incidence angle.
...
*   The parameter MXNBMC must equal MXGNAM (sequence MAGBPARM) !
     ...
     -      IPTECH,LREPSK,LKINK,LSTMAX,LEQSRT,LEQCRS,LEQMRK,LAVPRO
       REAL PHI,XW,YW
       DOUBLE PRECISION X0,Y0,Z0,CT,ST,CP,SP,UU,VV
       INTEGER IW,ISOLID,IREF
*** Deal with wires.
       IF(ISTAT.GE.1.AND.ISTAT.LE.NWIRE)THEN
            IW=ISTAT
            PHI=ATAN2(REAL(YU(NU))-Y(IW),REAL(XU(NU)-X(IW)))
*** Deal with wire replicas.
       ELSEIF(ISTAT.GE.MXWIRE+1.AND.ISTAT.LE.MXWIRE+NWIRE)THEN
            IW=ISTAT-MXWIRE
            XW=X(IW)
            IF(PERX)XW=XW+SX*ANINT((REAL(XU(NU))-XW)/SX)
            YW=Y(IW)
            IF(PERY)YW=YW+SY*ANINT((REAL(YU(NU))-YW)/SY)
            PHI=ATAN2(REAL(YU(NU))-YW,REAL(XU(NU))-XW)
*** Deal with solids.
       ELSEIF(ISTAT.GE.2*MXWIRE+1.AND.ISTAT.LE.2*MXWIRE+NSOLID)THEN
            ISOLID=ISTAT-2*MXWIRE
**  Cylinders can be processed in detail.
            IF(ISOLTP(ISOLID).EQ.1)THEN
*   Starting point in buffer.
                 IREF=ISTART(ISOLID)
                 IF(IREF.LT.0.OR.IREF+8.GT.MXSBUF)THEN
                      PRINT *,' !!!!!! DLCPHI WARNING : Solid address'//
     -                     ' is out of range ; returning reference.'
                      RETURN
                 ENDIF
*   Obtain parameters of the cylinder.
                 X0=CBUF(IREF+3)
                 Y0=CBUF(IREF+4)
                 Z0=CBUF(IREF+5)
                 CT=CBUF(IREF+10)
...
*   Compute the U and V coordinates.
                 UU=+CP*CT*(XU(NU)-X0)+SP*CT*(YU(NU)-Y0)-ST*(ZU(NU)-Z0)
                 VV=-SP   *(XU(NU)-X0)+CP*   (YU(NU)-Y0)
*   Compute the angle.
                 IF(UU.NE.0.OR.VV.NE.0)THEN
                      PHI=REAL(ATAN2(VV,UU))
                 ELSE
                      PHI=0
                 ENDIF
**  Other shapes are not yet processed specially.
            ELSE
                 PHI=0
            ENDIF
*** Anything else.
