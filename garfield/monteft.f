CDECK  ID>, MONTEFT.
      SUBROUTINE MONTEFT(IFAIL)
*-----------------------------------------------------------------------
*   MONTEFT - Calculates collision events and updates diffusion and
...
*-----------------------------------------------------------------------
     ...
     -         LSYNCH,LINPRD,LUNOUT,JFAIL,JEXMEM
       DOUBLE PRECISION EPRM(4000000),S,RDUM,E1,T,T2,AP,E,
     -      CONST6,CONST7,CONST9,
     -      ZSTRT,TSSTRT,DCX1,DCY1,DCZ1,DCX2,DCY2,DCZ2,
     -      E100,DCX100,DCY100,DCZ100,
     -      BP,F1,F2,F3,F4,F5,F6,F8,F9,TLIM,TDASH,TSTOP,DRAND48,TTEM,
     -      A,R1,R2,R3,R4,R5,R9,R31,S1,S2,EI,ESEC,PHI0,THETA0,D,Q,U,
     -      CSQD,ARGZ,ARG1,W,ANEION,ANBT,EPRMBAR,E2PRM,EBAR,EERR
       INTEGER IESPECP(100),I,ID,INTEM,I100,NCOL,NELEC,NEION,NMXADD,
     -      NPONT,NCLUS,J1,IPRINT,JPRINT,ITER,IPLANE,IDUM,IE,IT,IPT,JCT,
     -      IFAIL
       EXTERNAL DRAND48
       logical lab20
*** Identify the procedure if requested.
       IF(LIDENT)PRINT *,' /// ROUTINE MONTEFT ///'
...
*   in this cycle if so output current results.
 ...
 944       FORMAT(/,2X,'PROGRAM STOPPED TOO MANY PRIMARIES IPRIM =',I7)
           PRINT *,' !!!!!! MONTEFT WARNING: Attachment too high;'//
...
*   Store position and energy at time plane =IPLANE.
                CALL TPLANE(T,E1,DCX1,DCY1,DCZ1,AP,BP,EMAG,IPLANE)
...
*   Take electrons from store (label 20 no longer used, RV 13/5/2005).
C  20           continue
                lab20=.false.
                X=XS(NPONT)
                Y=YS(NPONT)
...
C994       FORMAT(2X,' WARNING ENERGY =',F8.3,
C    -          ' LESS THAN ENERGY LOSS EI=',F8.3,
C    -          ' AT ITER=',I12,' DUE TO BINNING ERROR')
...
C9232      FORMAT(3X,'WARNING ARGZ= 0.0 AT ITER =',I10,' ID =',
C    -          I10,' E1=',E12.3)
           DCZ1=F6
           DCX1=F9*F5
           DCY1=F8*F5
