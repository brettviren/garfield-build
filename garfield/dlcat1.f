...
*   Cheat in case the point is located inside a wire.
     ...
     -           BX,BY,BZ,BTOT)
            D(ILOCRS)=DRES
            IF(LDEBUG)PRINT *,' ++++++ DLCAT1 DEBUG   : Drift-line',
...
*   In case this didn't help, just log the failure.
     ...
     -           (YU(IU)-YU(IU-1))**2+(ZU(IU)-ZU(IU-1))**2)
       ENDIF
       IF(IU.GT.1)CRUDE=CRUDE+STEP*(ATTVEC(IU)+ATTVEC(IU-1))/2.0
100    CONTINUE
       NFC=NU
*** Print a heading for the debugging output.
       IF(LDEBUG)THEN
            PRINT *,' ++++++ DLCAT1 DEBUG   : Attachment integration',
...
*   Cheat in case the point is located inside a wire.
     ...
     -           BXM,BYM,BZM,BTOTM)
            NFC=NFC+1
            D(ILOCRS)=DRES
            IF(LDEBUG)PRINT *,' ++++++ DLCAT1 DEBUG   : Intermediate',
...
*   Add the new term to the integral.
            BETINT=BETINT+STEP*(BETA1+4.0*BETAM+BETA2)/6.0
